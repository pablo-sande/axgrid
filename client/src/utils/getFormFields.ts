import { ControlType, EnergyTypes, mandatoryFields, validateDynamicFormFieldsObj } from '../types/types'
import { data } from '../data/data.ts'

export const getFormFields = (energyType: EnergyTypes) => {
    if (!(data && data.energyTypes && data.energyTypes.length && data.default && data.default.length)) {
        return null
    }
    const values =
        (energyType &&
    ...
    const valuesWithInputType = allValues.map((_) => ({
        ..._,
        inputType: _.inputType as ControlType,
    }))
